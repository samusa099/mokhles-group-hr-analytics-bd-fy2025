...
def build() -> None:
    ...
    employee_headers, employees = read_table("01_Employee_Master_FY2025.csv")
    separation_headers, separations = read_table("06_Employee_Separations_FY2025.csv")
    ...
    write_table(OUTPUT / "00_dim_department.csv", ["department_key", "department", "cost_center"], departments)

    locations = sorted({row["Location"] for _, rows in all_source.values() for row in rows if row.get("Location")})
    write_table(
    ...
    for row in employees:
        unified[row["Employee ID"]] = {
            "employee_id": row["Employee ID"], "employee_name": row["Employee Name"],
            "gender": row["Gender"], "age": row["Age"], "age_band": row["Age Band"],
            "division_of_origin": row["Division of Origin"], "disability_status": row["Disability Status"],
            "education": row["Education"], "department": row["Department"],
            "department_key": normalise(row["Department"]), "cost_center": row["Cost Center"],
            "job_title": row["Job Title"], "job_level": row["Job Level"],
            "employment_type": row["Employment Type"], "location": row["Location"],
            "location_key": normalise(row["Location"]), "join_date": row["Join Date"],
            "exit_date": "", "employment_status": "Active at FY2025 year-end",
            "manager": row["Manager"], "monthly_gross_bdt": row["Monthly Gross"],
        }
    for row in separations:
        unified[row["Employee ID"]] = {
            "employee_id": row["Employee ID"], "employee_name": row["Employee Name"],
            "gender": row["Gender"], "age": "", "age_band": "", "division_of_origin": "",
            "disability_status": "", "education": "", "department": row["Department"],
            "department_key": normalise(row["Department"]),
            "cost_center": department_to_cost.get(row["Department"], ""),
            "job_title": row["Job Title"], "job_level": row["Job Level"],
            "employment_type": "", "location": row["Location"],
            "location_key": normalise(row["Location"]), "join_date": row["Join Date"],
            "exit_date": row["Exit Date"], "employment_status": "Separated during FY2025",
            "manager": row["Manager"], "monthly_gross_bdt": "",
        }
    employee_fields = list(next(iter(unified.values())).keys())
    write_table(OUTPUT / "01_dim_employee_fy2025.csv", employee_fields, [unified[key] for key in sorted(unified)])

    dates: list[date] = []
    for headers, rows in all_source.values():
        for header in headers:
            if "Date" in header or header == "Month":
                dates.extend(parsed for row in rows if (parsed := parse_date(row.get(header, ""))))
    current, end = min(dates), max(dates)
    calendar: list[dict[str, object]] = []
    while current <= end:
        quarter = (current.month - 1) // 3 + 1
        calendar.append({
            "date_key": int(current.strftime("%Y%m%d")), "date": current.isoformat(),
            "year": current.year, "quarter": f"Q{quarter}",
            "year_quarter": f"{current.year}-Q{quarter}", "month_number": current.month,
            "month_name": current.strftime("%B"), "month_short": current.strftime("%b"),
            "year_month": current.strftime("%Y-%m"), "month_start": current.replace(day=1).isoformat(),
            "week_of_year": int(current.strftime("%W")) + 1, "day_of_month": current.day,
    ...
    for source_name, output_name in TABLES.items():
        ...
        output_rows = [{normalise(header): row.get(header, "") for header in headers} for row in rows]
        write_table(OUTPUT / output_name, output_headers, output_rows)
        mapping_rows.extend({
            "source_file": source_name, "bi_ready_file": output_name,
            "original_field": original, "bi_ready_field": renamed,
    ...
    print(f"BI-ready layer created: {len(list(OUTPUT.glob('*.csv')))} CSV files")
